...
def initialize_schema(conn):
    ...
    -- Audit logging table for tracking business-critical operations
...
def seed_data(conn):
    ...
    # Seed security demo data
    seed_security_data(conn)


def seed_security_data(conn):
    """Seed security demo tables with sample data"""
    cursor = conn.cursor()

    cursor.execute("SELECT COUNT(*) FROM security_users")
    if cursor.fetchone()[0] > 0:
        print("Security data already exists, skipping")
        cursor.close()
        return

    print("Seeding security demo data...")

    security_users = [
        ("admin", "admin@example.com", "admin", "System administrator with full access"),
        ("john_doe", "john@example.com", "user", "Regular user account"),
        ("jane_smith", "jane@example.com", "editor", "Content editor"),
        ("bob_wilson", "bob@example.com", "user", "Marketing team member"),
        ("alice_jones", "alice@example.com", "moderator", "Community moderator"),
    ]

    for user in security_users:
        cursor.execute(
            "INSERT INTO security_users (username, email, role, bio) VALUES (%s, %s, %s, %s) ON CONFLICT DO NOTHING",
            user,
        )

    security_comments = [
        (1, "Welcome to the security demo! Try posting comments with XSS payloads."),
        (2, "This is a normal comment from John."),
        (3, "Great demo application for learning about web security!"),
        (4, "Remember: always sanitize user input in production applications."),
        (5, "The AWS Security Agent can detect these vulnerabilities automatically."),
    ]

    for comment in security_comments:
        cursor.execute(
            "INSERT INTO security_comments (user_id, content) VALUES (%s, %s)",
            comment,
        )

    conn.commit()
    cursor.close()
    print("✓ Security demo data seeded")
