...
Vulnerability inventory (matches pen-test target set):
  ...
  7. Mass Assignment        — POST /security-comments     [FIXED] (no longer accepts author_name/role from body)
...
# 3. Stored XSS — Comments endpoints (Mass Assignment FIXED)
...
def create_security_comment(connection, body):
    ...
    try:
        ...
        # FIXED: Mass assignment vulnerability removed - only accept user_id and content
        # author_name and role fields are no longer accepted from request body
        cursor.execute(
