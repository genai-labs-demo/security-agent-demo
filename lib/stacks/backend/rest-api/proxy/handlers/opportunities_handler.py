...
def _recalculate_account_aggregates(connection, account_id: str) -> None:
    ...
    on the parent account after any opportunity create/update/delete. This function must be
    called within a transaction context - it does not commit. The caller is responsible for
    committing the transaction to ensure atomicity with the opportunity operation.
    ...
    try:
        ...
        logger.info(f"Recalculated aggregates for account {account_id} (pending commit)")
    except Exception as e:
        logger.error(f"Failed to recalculate aggregates for account {account_id}: {str(e)}")
        # Re-raise the exception so the caller can rollback the entire transaction
        # This ensures atomicity between opportunity operations and aggregate updates
        raise
    finally:
        cursor.close()
...
def create_opportunity(connection, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Recalculate parent account aggregates before committing
